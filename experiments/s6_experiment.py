...
QUERY_FILE = Path("docs/experiments/S6/QUERY_SET.md")
FALLBACK_QUERY_FILE = Path("docs/experiments/S5/QUERY_SET.md")
TOP_K = 3
TIMEOUT = 180.0


def load_queries():
    qf = QUERY_FILE if QUERY_FILE.exists() else FALLBACK_QUERY_FILE
    if not qf.exists():
...
def health_check(client):
    print("\n" + "=" * 70)
    print("  ARYNTRA SYNAPSE - S6 SEMANTIC SUFFICIENCY EXPERIMENT")
    print("=" * 70)
    print("\n[1/3] Health Check")
    print("-" * 40)
...
def run_queries(client, queries, mode):
    print(f"\n[2/3] Running S6 Query Set ({mode})")
    print("-" * 40)
    results = []
    for item in queries:
        ...
        try:
            ...
            print(
                ...
                f"Steps: {result['expansion_steps']} | "
                f"Calls: {result['total_model_calls']}"
            )
            print(
                f"  Active: {result['workspace_active_chunks']} | "
                f"Final Ctx: {result['final_context_length']} | "
                f"Latency: {result['total_latency']}s"
            )

            # Print semantic scores if available
            if result["sufficiency_log"]:
                last = result["sufficiency_log"][-1]
                if "semantic_score" in last:
                    print(
                        f"  Semantic: {last.get('semantic_score', 'N/A')} | "
                        f"Lexical: {last.get('lexical', {}).get('coverage_ratio', 'N/A')} | "
...
def save_results(health, results, mode):
    result_file = Path(f"experiments/S6_results_{mode}.json")
    output = {
        "experiment": "S6",
        "phase": "semantic-sufficiency-evaluation",
        "query_set": "S6 Query Set v1",
        "context_representation": mode,
        "top_k": TOP_K,
        "health": health,
        "results": results,
    }
...
def print_summary(results, mode):
    ...
    early_stops = sum(
        1 for r in results
        if r.get("stop_reason") == "evidence_sufficient"
    )
    print(f"\n[3/3] Summary ({mode}): {passed}/{len(results)} passed")
    print(f"  Early stops: {early_stops}/{len(results)}")
    print("=" * 70)
...
def main():
    ...
    with httpx.Client() as client:
        ...
        mode = health.get("context_representation", "unknown")
        results = run_queries(client, queries, mode)
        save_results(health, results, mode)
