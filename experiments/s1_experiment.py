"""
Aryntra Synapse — S1 Structured Context Experiment Runner

Purpose:
    Execute the canonical S1 Query Set v1 against the active Synapse configuration
    (e.g., CONTEXT_REPRESENTATION=structured_v1) and output S1_results_v1.json.
"""
...
RESULT_FILE = Path("experiments/S1_results_v1.json")
BASELINE_FILE = Path("experiments/S1_baseline_results_v1.json")
TOP_K = 3
TIMEOUT = 120.0
...
def health_check(client):
    print("\n" + "=" * 70)
    print("  ARYNTRA SYNAPSE — S1 STRUCTURED EXPERIMENT")
    print("=" * 70)
    print("\n[1/3] Health Check")
    print("-" * 40)
    ...
    for item in queries:
        ...
        try:
            ...
            result = {
                ...
                "representation_build_latency": data.get("representation_build_latency"),
...
def print_summary(results):
    ...
    if BASELINE_FILE.exists():
        try:
            ...
            print("\n" + "=" * 70)
            print("  BASELINE vs S1 STRUCTURED COMPARISON (LATENCY & CONTEXT)")
            print("=" * 70)
            print(f"{'ID':<5} | {'Base Gen (s)':<13} | {'S1 Gen (s)':<13} | {'Base Ctx':<10} | {'S1 Ctx':<10}")
            print("-" * 60)
            for r in results:
                qid = r["id"]
                b = b_results.get(qid, {})
                ...
                print(f"{qid:<5} | {b_gen:<13} | {s_gen:<13} | {b_ctx:<10} | {s_ctx:<10}")
        except Exception as e:
            print(f"Could not load baseline comparison: {e}")

    print("\n" + "=" * 70)
    print("  S1 EXPERIMENT COMPLETE")
    print("=" * 70)
